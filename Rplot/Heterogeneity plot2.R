...
X7111$id <- factor(X7111$id, levels = c("State-owned", "Private","Labor-intensive", "Technology-intensive", "Capital-intensive", "Heavy polluting", "Non-heavy polluting"))

# 创建置信区间可视化图
ggplot(X7111, aes(x = id, y = coef, color = id)) +
  ...
  theme(
    ...
    axis.text.x = element_text(angle = -20, hjust = 0.35),  # 设置 X 轴刻度标签大小、角度和位置
  ...
  coord_cartesian(ylim = c(-14, 6)) + # 设置y轴范围
  scale_color_manual(
    values = c(
      "State-owned" = "#367DC0",
      "Private" = "#367DC0",
      "Labor-intensive" = "orange",
      "Technology-intensive" = "orange",
      "Capital-intensive" = "orange",
      "Heavy polluting" = "green3",
      "Non-heavy polluting" = "green3"
    ),
    labels = c(
  ...
  labs(color = "Business grouping") # 设置图例标题
